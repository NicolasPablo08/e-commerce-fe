...
export function BlueButton({ children, className, onClick, type }: ButtonsProps) {
  return (
    <DefaultButton type={type} onClick={onClick} className={`bg-[var(--azul-800)] ${className}`}>
      {children}
    </DefaultButton>
  );
}
export function YellowButton({ children, className, onClick, type }: ButtonsProps) {
  return (
    <DefaultButton
      type={type}
      onClick={onClick}
...
export function CelesteButton({ children, className, onClick, type }: ButtonsProps) {
  return (
    <button
      type={type}
      onClick={onClick}
      className={`rounded-[8px] bg-[var(--celeste-A100)] block ${className}`}
    >
